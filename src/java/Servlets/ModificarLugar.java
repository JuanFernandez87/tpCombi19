...
public class ModificarLugar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ModificarLugar</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ModificarLugar at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(!existe && !viajeActivo){ //si el lugar no existe se modifica
            control.modificarLugar(idLugar, nombre, provincia);
            response.sendRedirect ("popUpRegistroCorrectoLugar.jsp");   
            
        }else if(viajeActivo){
            response.sendRedirect ("popUpErrorLugarConViaje.jsp"); 
